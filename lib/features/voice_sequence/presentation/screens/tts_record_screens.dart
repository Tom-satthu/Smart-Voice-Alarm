...
class _TtsScreenState extends ConsumerState<TtsScreen> {
  ...
  void _save() {
    ...
    final voice = voices.firstWhere((v) => v.id == _selectedVoiceId);

    ref.read(voiceSequenceProvider.notifier).add(
          VoiceSegmentUiModel(
            id: const Uuid().v4(),
            name: text.length > 28 ? '${text.substring(0, 28)}…' : text,
            type: VoiceSegmentType.tts,
            duration: Duration(seconds: (text.split(' ').length * 0.4).ceil().clamp(3, 60)),
            text: text,
          ),
        );

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Saved with ${voice.name}')),
    );
    context.pop();
  }
    ...
    return AppScaffold(
      ...
      body: ResponsiveCenter(
        child: ListView(
          ...
          children: [
            SectionHeader(title: l10n.ttsTitle),
            TextField(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(voice.name, style: context.textTheme.titleSmall),
                            Text(
                              voice.locale,
                      ...
                      if (voice.isPremium)
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: context.colors.secondary.withValues(alpha: 0.18),
...
class _RecordScreenState extends ConsumerState<RecordScreen> {
  ...
  Future<void> _start() async {
    ...
    for (var i = 0; i < 40 && mounted && _phase == _RecordPhase.recording; i++) {
  ...
  Future<void> _play() async {
    if (_phase != _RecordPhase.ready && _phase != _RecordPhase.playing) return;
  ...
  void _save() {
    ref.read(voiceSequenceProvider.notifier).add(
          VoiceSegmentUiModel(
            id: const Uuid().v4(),
            name: 'Recording ${_elapsed.compact}',
  ...
  Widget build(BuildContext context) {
    ...
    final canSave = _phase == _RecordPhase.ready || _phase == _RecordPhase.playing;
            ...
            children: [
              ...
              Text(
                switch (_phase) {
                  ...
                  _RecordPhase.playing => l10n.recordPlay,
                },
              ...
              const SizedBox(height: AppConstants.spaceXl),
              WaveVisualizer(active: isRecording || _phase == _RecordPhase.playing),
              const Spacer(),
              Row(
